...
نسخة Python / Streamlit متجاوبة مع متصفح الجوال.

طريقة التشغيل:
1) pip install streamlit pandas
2) streamlit run smart_accreditation_streamlit_app_updated.py
"""
...
# =========================================================
# 1) مكتبة الشواهد العامة حسب المجال
# =========================================================

EVIDENCE_LIBRARY: Dict[str, Dict[str, List[str] | str]] = {
    "governance": {
...
# =========================================================
# 2) تعريف المعايير والمحكات
# =========================================================

def criterion(code: str, text: str, themes: List[str], essential: bool = False) -> Dict[str, Any]:
    return {
        "code": code,
        "text": text,
        "themes": themes,
...
def flatten_criteria() -> List[Dict[str, Any]]:
    rows = []
...
def support_for_criterion(themes: List[str]) -> Dict[str, List[str]]:
    evidences: List[str] = []
    tips: List[str] = []
    labels: List[str] = []
    ...
    for theme in themes:
        ...
        evidences.extend(item.get("evidences", []))  # type: ignore[arg-type]
        tips.extend(item.get("tips", []))  # type: ignore[arg-type]
...
def init_session_state(rows: List[Dict[str, Any]]) -> None:
    if "readiness" not in st.session_state:
        st.session_state.readiness = {
            row["criterion_code"]: "غير مقيّم" for row in rows
        }

    if "notes" not in st.session_state:
        st.session_state.notes = {
            row["criterion_code"]: "" for row in rows
        }


def calculate_score(rows: List[Dict[str, Any]]) -> int:
    if not rows:
        return 0
    ...
    for row in rows:
        ...
        total += STATUS_OPTIONS.get(status, 0)

    return round((total / len(rows)) * 100)


def generate_criterion_report(row: Dict[str, Any]) -> str:
...
def generate_gap_report(rows: List[Dict[str, Any]]) -> str:
    ...
    not_evaluated = sum(1 for row in rows if st.session_state.readiness[row["criterion_code"]] == "غير مقيّم")

    lines = [
        "تقرير فجوات الشواهد للاعتماد البرامجي",
        f"تاريخ التقرير: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        f"نسبة الجاهزية العامة: {score}%",
        f"إجمالي المحكات: {len(rows)}",
        f"مكتمل: {complete}",
        f"جزئي: {partial}",
        f"غير متوفر: {missing}",
        f"غير مقيّم: {not_evaluated}",
        "",
    ]
...
def main() -> None:
    ...
    metrics_html = f"""
    <div class="metrics-grid" dir="rtl">
        <div class="metric-card">
            <div class="metric-label">نسبة الجاهزية</div>
            <div class="metric-value">{score}%</div>
        </div>
        <div class="metric-card">
            <div class="metric-label">إجمالي المحكات</div>
            <div class="metric-value">{len(rows)}</div>
        </div>
        <div class="metric-card">
            <div class="metric-label">المحكات الأساسية</div>
            <div class="metric-value">{essential_count}</div>
        </div>
        <div class="metric-card">
            <div class="metric-label">مكتمل</div>
            <div class="metric-value">{complete}</div>
        </div>
        <div class="metric-card">
            <div class="metric-label">جزئي/غير متوفر</div>
            <div class="metric-value">{partial + missing}</div>
        </div>
    </div>
    """
    ...
    with st.sidebar:
        ...
        filter_status = st.selectbox(
            "فلترة حسب الجاهزية",
            ["الكل", "غير مقيّم", "مكتمل", "جزئي", "غير متوفر"],
        )
    ...
    if filter_status != "الكل":
        visible_rows = [
            ...
            if st.session_state.readiness[row["criterion_code"]] == filter_status
        ]
    ...
    criterion_options = [
        f"{row['criterion_code']} - {row['criterion_text'][:70]}"
    ...
    essential_badge = (
        '<span class="essential-badge">محك أساسي</span>'
        if selected_row["essential"]
        else ""
    )
    ...
    st.info(
        "تنبيه مراجع جودة: الشاهد القوي ليس الملف الأكثر عددًا، بل الملف الأوضح صلة بالمحك، "
        "والأثبت اعتمادًا، والأقدر على إظهار التحليل والتحسين."
    )
