...
نسخة Python / Streamlit محسّنة للجوال وبدون st.sidebar.
...
def main() -> None:
    ...
    essential_label = "محك أساسي" if selected_row["essential"] else ""

    st.markdown(
        f"""
        <div class="criterion-card">
            <span class="criterion-code">{selected_row["criterion_code"]}</span>
            <span class="criterion-code">{selected_row["substandard"]}</span>
            {'<span class="criterion-essential">محك أساسي</span>' if selected_row["essential"] else ''}
            <div class="criterion-text">{selected_row["criterion_text"]}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.download_button(
        ...
        data=generate_criterion_report(selected_row),
